...
import { Palette, Clock, DollarSign, FileText, AlertTriangle } from 'lucide-react'
...
interface CreateConsultationTypeDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  onCreate: (data: CreateConsultationTypeInput) => Promise<void>
  loading: boolean
}

export function CreateConsultationTypeDialog({
  open,
  onOpenChange,
  onCreate,
  loading
}: CreateConsultationTypeDialogProps) {
  const [selectedColor, setSelectedColor] = useState(CONSULTATION_TYPE_COLORS[0].value)

  const form = useForm<CreateConsultationTypeInput>({
    resolver: zodResolver(createConsultationTypeSchema),
    defaultValues: {
      name: '',
      description: '',
      duration: 30,
      price: undefined,
      color: CONSULTATION_TYPE_COLORS[0].value,
  ...
  const handleSubmit = async (data: CreateConsultationTypeInput) => {
    await onCreate(data)
    form.reset()
    setSelectedColor(CONSULTATION_TYPE_COLORS[0].value)
    onOpenChange(false)
  }
